...
use paged_renderer::{BytesResolver, Document};
...
fn viewer_session_load_opens_container_and_builds_pages() {
    ...
    let document = Document::open(&bytes).expect("open IDML container");
    let built =
...
fn viewer_session_load_reports_errors_without_panicking() {
    ...
    let opened = Document::open(&garbage);
    assert!(
        opened.is_err(),
