...
pub fn resolve(
    ...
    // Footnote count — model surfaces it from idml-scene's anchor
    ...
    result.footnote_count = scene
        .anchors
        .iter()
        .filter(|a| matches!(a.kind, idml_scene::AnchorKind::FootnoteBody))
        .count();
...
<TextFrame Self="tf1" ParentStory="story1" GeometricBounds="100 100 400 400" ItemTransform="1 0 0 1 0 0"/>
            ...
            // Story filename must match the Self attribute (idml-scene
