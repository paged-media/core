...
//! diff through `idml_mutate::apply` so the unified undo log gets a
...
use idml_mutate::{NodeId, NodeSpec, Operation, PropertyPath, Value};
...
pub struct NodeSnapshot {
    ...
    /// every update.
    pub(crate) path_anchors: Vec<idml_parse::PathAnchor>,
}
...
/// `idml_mutate::Operation`. Splitting bounds vs transform here means
...
impl CanvasModel {
    ...
    /// delta through `idml_mutate::apply` so the unified undo log
    ...
    pub fn commit_gesture(
        ...
        let Some(delta) = session.current_delta else {
            ...
            // stays uniform.
            let empty = idml_mutate::AppliedOperation {
        ...
        // mutations no longer exist as far as `idml_mutate::apply` is
    ...
    match session.gesture {
        ...
        GestureType::PathEdit { address } => {
            ...
            let anchor = snap
                ...
                .get(address.index)
                .copied()
                .unwrap_or(idml_parse::PathAnchor {
                    anchor: (0.0, 0.0),
                    left: (0.0, 0.0),
                    right: (0.0, 0.0),
                });
            let original = match address.role {
                idml_mutate::PathPointRole::Anchor => anchor.anchor,
                idml_mutate::PathPointRole::Left => anchor.left,
                idml_mutate::PathPointRole::Right => anchor.right,
            };
...
fn collect_sibling_frames(
    scene: &idml_scene::Document,
    built: &idml_renderer::BuiltDocument,
                    ...
                    .find(|p| p.self_id.as_deref() == Some(id.as_str()))
                {
                    if let NodeMutation::PathPoint { address, position } = mutation {
                        if let Some(anchor) = p.anchors.get_mut(address.index) {
                            match address.role {
                                idml_mutate::PathPointRole::Anchor => {
                                ...
                                idml_mutate::PathPointRole::Left => {
                                    anchor.left = (position[0], position[1]);
                                }
                                idml_mutate::PathPointRole::Right => {
...
fn restore_snapshot_in_scene(scene: &mut idml_scene::Document, snap: &NodeSnapshot) {
    match &snap.node_id {
        NodeId::TextFrame(id) => {
            for parsed in scene.spreads.iter_mut() {
                if let Some(f) = parsed
...
fn duplicate_suffix() -> String {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        // wasm binding, but idml-canvas is pure Rust and doesn't
