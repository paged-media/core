...
fn emit_footnote_pools(
    ...
    for page in pages.iter_mut() {
        ...
        for (_key, group) in by_frame {
            ...
            // frame's bottom.
            let mut layouts: Vec<paged_text::LaidOutParagraph> = Vec::with_capacity(group.len());
            for fn_ in &group {
                let text = footnote_body_text(fn_);
                let mut lopts = paged_text::LayoutOptions::new(column_width_pt, point_size);
                lopts.alignment = paged_text::Alignment::Left;
                let shaper = paged_text::RustybuzzMeasurer::new(&rb_face, point_size);
...
fn unit_ellipse_path() -> paged_compose::PathData {
    use paged_compose::PathSegment;
...
fn decode_image_bytes(bytes: &[u8]) -> Option<paged_compose::DecodedImage> {
