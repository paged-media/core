...
fn per_run_font_switch_uses_both_fonts() {
    let document = Document::open(&build_per_run_idml()).unwrap();
...
fn underline_adds_ink_relative_to_plain_run() {
    ...
    let plain_doc = Document::open(&build_underline_idml(false)).unwrap();
    let underlined_doc = Document::open(&build_underline_idml(true)).unwrap();
...
<idPkg:Story xmlns:idPkg="http://ns.adobe.com/AdobeInDesign/idml/1.0/packaging">
  <Story Self="u10">
    ...
    let bullet_doc = Document::open(&bullet_bytes).unwrap();
