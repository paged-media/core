...
fn main() {
    ...
    for path in shaders {
        ...
        let source = match std::fs::read_to_string(path) {
            Ok(s) => s,
            Err(e) => panic!("paged-gpu build.rs: cannot read {path}: {e}"),
        };
        let module = match naga::front::wgsl::parse_str(&source) {
            Ok(m) => m,
            Err(e) => {
                let msg = e.emit_to_string(&source);
                panic!("paged-gpu build.rs: WGSL parse error in {path}:\n{msg}");
            }
        };
        ...
        if let Err(e) = validator.validate(&module) {
            let msg = e.emit_to_string(&source);
            panic!("paged-gpu build.rs: WGSL validation error in {path}:\n{msg}");
        }
    }
}
